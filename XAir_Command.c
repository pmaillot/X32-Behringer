...
// XAir_Command.c
//
//  Created on: Sep 19, 2014 (X32_Command.c)
//
//      XAir_Command: a simple udp client for XAir sending commands and getting answers
//
...
// v 1.35: Removed "-s" option as XAir doesn't support "/" node format. 
    ...
    if (xremote_on) {                                                            \
        ...
        if (now > before + XREMOTE_TIMEOUT) {                                     \
            ...
            if (sendto (Xfd, xremote, 12, 0, Xip_addr, Xip_len) < 0) {            \
                perror ("couldn't send data to XAir");                            \
                exit (EXIT_FAILURE);                                            \
            }                                                                    \
            before = now;                                                        \
        }                                                                        \
...
//          break;
        ...
        default:
        case 'h':
            printf("usage: XAir_Command [-i XAir console ipv4 address]\n");
...
//          printf(" until EOF has been reached, and exits after that.\n\n");
            ...
            printf(" All other commands are parsed and sent to XAir.\n");
            printf(" Typical XAir command structure:\n");
...
// Establish logical connection with XR18 server
    printf(" XAir_Command - v1.34 - (c)2014-15 Patrick-Gilles Maillot\n\nConnecting to XAir.");
//
    keep_on = 1;
        ...
        if (filein == 1) {
            while (keep_on) {
                ...
                if (fgets(input_line, LINEMAX, fdk)) {
                    ...
                    CHECKX32()            // Check if XR18 sent something back
                } else {
                    keep_on = 0;
                }
            }
            ...
            while (fgets(input_line, LINEMAX, fdk) != NULL) {
                ...
                if (input_line[0] != '#') {
                    ...
                    CHECKX32()        // XR18 will echo back the line
                }
            }
            printf ("---end of file\n");
            fflush (stdout);
